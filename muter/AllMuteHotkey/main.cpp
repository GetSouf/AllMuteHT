...
int run_cli(int argc, wchar_t** argv) {
    ...
    if (argc < 2) {
        ...
        } else if (!amh::is_daemon_running()) {
            amh::print_info(L"Фоновый режим не запущен. Выполните: AllMuteHotkey start");
        }
        return 0;
    }
    ...
    if (command == L"hotkey") {
        ...
        HWND probe = CreateWindowExW(0, L"STATIC", L"", 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, nullptr,
                                     nullptr);
        if (!probe) {
    ...
    if (command == L"start" || command == L"run") {
        ...
        if (command == L"start") {
            ...
            if (!CreateProcessW(nullptr, cmd_buffer.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW,
    ...
    if (command == L"install") {
        ...
        if (!amh::is_daemon_running()) {
            ...
            si.dwFlags = STARTF_USESHOWWINDOW;
            si.wShowWindow = SW_HIDE;
            CreateProcessW(nullptr, cmd_buffer.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
                           nullptr, &si, &pi);
            if (pi.hProcess) {
                CloseHandle(pi.hThread);
                CloseHandle(pi.hProcess);
